...
Non-deterministic behaviour in node-stopping mechanisms used to stage upgrades: both _halt-height_ and _Cosmovisor add-upgrade_ resulted in different hashes.
...
- Hypha: Test impact of pruning configuration in rollback command
- Hypha: Write up for possibly integrating the multiplexer into the sdk
- ICL: Put up forum prop to modify voting period in gov parameters: 7 days for normal props, 3 days for expedited props
- Hypha + ICL: Publish upgrade blog post
...
- Rollback mechanism was recently tested and demonstrated in testnet Tuesday events
...
- Breakdown in communication between Hypha + ICL about what was state breaking at what point
...
| ---------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2025-04-09 20:03 | Hypha & ICL | Agreement to schedule an emergency upgrade to address a sensitive security issue for Monday, April 14.                                                                                               |
...
| 2025-04-11 17:12 | Hypha       | Upgrade test passes local test.                                                                                                                                                                      |
...
| 2025-04-14 15:55 | Hypha   | Anniynces A1 is the canonical hash and posts rollback instructions for validators with BB hash.                                                                                                                                                            |
...
| Upgrade staging mechanism | A1 | BB |
| :--- | :-: | :-: |
...
* 7 of 28 validators that used `halt-height` (both via app.toml and start flag) got the non-canonical hash
* 3 of 7 validators that used the Cosmovisor `add-upgrade` got the non-canonical hash
